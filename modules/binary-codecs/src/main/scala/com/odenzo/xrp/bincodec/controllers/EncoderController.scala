...
/**
  * Top Level Decoding of a full message, expecting to be an STObject with no
  * marker. Will need to replicate in STObject Encoder with extra markers
  */
object EncoderController extends MyLogging {
  ...
  /**
    ...
    * out those that are not signing. Then looks up the encoder and does the
    * encoding. Returns the whole
    * @return
