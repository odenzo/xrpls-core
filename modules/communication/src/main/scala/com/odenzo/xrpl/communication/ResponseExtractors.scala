...
object ResponseExtractors {
  ...
  def extractStatus(rs: Json): IO[XrplStatus] = {
    IO.fromOption(root.status.as[XrplStatus].getOption(rs))(IllegalStateException(s"No Status Found"))
  }
  ...
  def extractResult[RS: Decoder](rs: Json): IO[RS] = {
    for {
      result <- IO.fromOption(root.json.getOption(rs))(IllegalStateException(s"No Result Found in Response"))
