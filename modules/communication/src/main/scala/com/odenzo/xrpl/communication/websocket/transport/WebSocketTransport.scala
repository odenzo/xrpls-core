package com.odenzo.xrpl.communication.websocket.transport
...
import com.odenzo.xrpl.communication.websocket.transport.WebSocketTransport.extractUUIDFromIdField
...
class WebSocketTransport(
  ...
  val sender: Stream[IO, Unit] = topic.subscribeUnbounded.evalMap { jo =>
    val sendProg: IO[Unit] = for {
      ...
      sent <- ws.send(frame) // Is this MT safe? if so parEval[Tap|Map]
