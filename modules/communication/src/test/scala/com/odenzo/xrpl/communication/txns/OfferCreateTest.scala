...
import com.odenzo.xrpl.models.data.atoms.{ AccountTxnNumber, XrplTime }
import com.odenzo.xrpl.models.data.monetary.*
import com.tersesystems.blindsight.LoggerFactory
import io.circe.syntax.*

class OfferCreateTest extends LocalCommsTest(TestScenarios.mode) {
  private val log = LoggerFactory.getLogger

  test("CreateOffer on GENESIS") {
    given engine: XrplEngine = engineFixture()
    log.info(s"Using Engine: ${engine.engineName}")
    ...
    for {
      ...
      _                  = log.info(s"Receiver Fiat Balances received")
...
//      _             = log.info(s"TxRs: ${txRs.asJson.spaces4}")
      ...
      _                  = log.fluent.info.argument(logArg("afterDeleted", afterDeleted))
    } yield ()
    end for
